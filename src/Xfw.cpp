...
namespace Xfw
{
	...
	{
		const bool windowStaysInTaskList = (groupWindow != nullptr && !groupWindow->getState(XFW_WINDOW_STATE_SKIP_TASKLIST));
		GtkWidget * const menu = windowStaysInTaskList ? xfw_window_action_menu_new(groupWindow->mXfwWindow) : gtk_menu_new();
		...
		if (!appInfo->path.empty())
		{
			...
			if(windowStaysInTaskList) // if the window entries exist, add a separator
				gtk_menu_shell_prepend(GTK_MENU_SHELL(menu), gtk_separator_menu_item_new());
