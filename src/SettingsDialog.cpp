		...
		if (path != nullptr)
		{
			g_signal_connect(shortcutsButton, "clicked",
				G_CALLBACK(+[](GtkButton* _shortcutsButton) {
					...
					if (g_spawn_command_line_sync("xfce4-keyboard-settings --help", &stdout, nullptr, nullptr, &error))
					{
						if (stdout != nullptr && g_strstr_len(stdout, -1, "--shortcuts") != nullptr)
							g_spawn_command_line_async("xfce4-keyboard-settings --shortcuts", &error);
						else
							g_spawn_command_line_async("xfce4-keyboard-settings", &error);
						g_free(stdout);
					}
					if (error != nullptr)
					{
						g_warning("Unable to start xfce4-keyboard-settings: %s", error->message);
						g_error_free(error);
					}
				}),
				nullptr);
