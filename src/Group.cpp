...
Group::Group(std::shared_ptr<AppInfo> appInfo, bool pinned) : mPinned(pinned), mActive(false), mWindowMenuShown(false), mTopWindowIndex(0), mAppInfo(appInfo), mGroupMenu(this), mIconPixbuf(nullptr), mContextMenu(nullptr)
{
	...
	g_signal_connect(G_OBJECT(mButton), "size-allocate",
...
void Group::resize()
{
	// The badge's current proportions look right on a 36 px panel. Scale all of its
	...
	const double launcherCountScale = CLAMP(Dock::mPanelSize / 36.0, 0.6, 1.25);
