...
struct AppInfo
{
	...
	const std::string mExec;
	const Store::AutoPtr<GDesktopAppInfo> mGAppInfo;
	AppInfo(std::string id, std::string path, std::string icon, std::string name, std::string exec, GDesktopAppInfo* gAppInfo = nullptr)
		: mId(id), mPath(path), mIcon(icon), mName(name), mExec(exec), mGAppInfo(gAppInfo, [](gpointer o) { if (o) g_object_unref(o); }) {}
